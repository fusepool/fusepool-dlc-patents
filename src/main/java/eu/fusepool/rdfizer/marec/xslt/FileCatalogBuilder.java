...
public class FileCatalogBuilder {
    ...
    public FileCatalogBuilder(File baseDirectory) {
        this.baseDirectory = baseDirectory;
        log.info("Storage folder: " + baseDirectory.getPath());
    }
    ...
    public void build() throws Exception {
        ...
        File dtdDir = new File(baseDirectory.getPath() + BASE_DTD_FOLDER);
        if(dtdDir != null && !dtdDir.exists())
            makeDTDDir() ;
        
        copyZipFile("dtd-patent.zip") ;
        String unzipFolderPath = BASE_ZIP_FOLDER + "dtd-patent.zip";
        unZipIt(unzipFolderPath, BASE_DTD_FOLDER) ;
    ...
    private void createProps() {
        try {
            ...
            File catalogProps = new File(baseDirectory.getPath() + "/CatalogManager.properties") ;
    ...
    private void createCatalog() throws Exception {
        ...
        try {
            ...
            String catalogPath = baseDirectory.getPath() + "/" + "catalog" + "/" + CATALOG_FILE;
            File fCatalog = new File(catalogPath) ;
            this.catalogPath = fCatalog.getAbsolutePath() ;
    ...
    private void copyZipFile(String zipRes) throws Exception {
        ...
        File zipDir = new File(baseDirectory.getPath() + "/" + zipPath) ;
        if(!zipDir.exists()) {
            zipDir.mkdirs() ;
        }
        ...
        File destFile = new File(baseDirectory.getPath() + "/" + zipPath + zipRes) ;
    ...
    private void makeBaseDir(String base) {
        ...
        File baseDir = new File(baseDirectory.getPath() + "/" + base);
        baseDir.mkdir() ;
    }

    private void makeDTDDir() {
        //File baseDir = bundleContext.getDataFile(BASE_DTD_FOLDER) ;
        File baseDir = new File(baseDirectory.getPath() + "/" + BASE_DTD_FOLDER) ;
        baseDir.mkdir() ;
    ...
    public void unZipIt(String zipFile, String outputFolder)    {
        ...
        try{
            ...
            File srcFile = new File(baseDirectory.getPath() + "/" + zipFile) ;
            ...
            while (entries.hasMoreElements()) {
                ...
                File newFile = new File(baseDirectory.getPath() + "/" + outputFolder + fileName) ;
